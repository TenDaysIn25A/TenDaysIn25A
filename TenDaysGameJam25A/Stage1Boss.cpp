...
void Stage1Boss::Initialize() {
	...
	hpGauge.Initialize();
	hpGauge.CreateHpGauge({ 0.0f,300.0f }, hp, maxHp, 500.0f, 60.0f, color, true);
...
void Stage1Boss::AnimUpdate() {
	if (input.GetKeyTrigger(DIK_SPACE)) {
		if (chochinColor == 0x00FFFFFF) {
			chochinColor = 0xFFFF00FF;
		} else {
			chochinColor = 0x00FFFFFF;
		}
	}
				...
				for (int i = 0; i < kBulletMax; i++) {
					if (!bullets[i].effect.GetIsActive()) {
						if (!bullets[i].isActive) {
							//ウェーブ
							InitializeBullets(i, { .height = 80.0f,.grHandle = grHandleBulletFish});
							bullets[i].ShotDir({ 640.0f + (bullets[i].width),120.0f - (80.0f * static_cast<float>(randomPosition)) }, { -1.0f, 0.0f }, 0.0f);
							break;
						}
					}
				}
