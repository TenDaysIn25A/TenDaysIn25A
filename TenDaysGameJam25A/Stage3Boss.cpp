...
Stage3Boss::Stage3Boss() { Initialize(); }

void Stage3Boss::Initialize() {
	speed = 10.0f;
	width = 128.0f;
	height = 480.0f;
	isAlive = true;
...
void Stage3Boss::InitializeBullets(int index, const BulletConfig& bulletConfig) {
...
void Stage3Boss::SetCamera(const Transform2D& camera) { renderer.SetCamera(camera); }

void Stage3Boss::Move() {

}

void Stage3Boss::TakeDamage(int damage) {
	if (!isAlive) {
		return;
	}
...
void Stage3Boss::Shot() {
	switch (attack) {
	case Stage3BossAttack::WALL:
		AttackWall();
		break;
	case Stage3BossAttack::MACHINGUN:
		AttackMachingun();
		break;
	case Stage3BossAttack::TUNNEL:
		AttackFishBone();
		break;
	case Stage3BossAttack::ALL_WALL:
		AttackAllWall();
		break;
	case Stage3BossAttack::FOURWALL:
		AttackFourWall();
		break;
	}
...
void Stage3Boss::CommonAttackSelect() {
	switch (attackPhase) {
	case AttackPhase::FIRST:
		attack = Stage3BossAttack::WALL;
		break;
	case AttackPhase::SECOND:
		attack = Stage3BossAttack::MACHINGUN;
		break;
	case AttackPhase::THIRD:
		attack = Stage3BossAttack::FOURWALL;
		break;
	}
}

void Stage3Boss::SpecialAttackSelect() {
	int randomAttack;
	switch (attackPhase) {
	case AttackPhase::FIRST:
		...
		if (randomAttack == 1) {
			attack = Stage3BossAttack::ALL_WALL;
		} else if (randomAttack == 2) {
			attack = Stage3BossAttack::TUNNEL;
		} else {
		}
		break;
	case AttackPhase::SECOND:
...
void Stage3Boss::AttackFourWall() {
	randomPositionY;
...
void Stage3Boss::AttackMachingun() {
	if (shotTimer >= 40) {
		shotTimer = 0;
...
void Stage3Boss::AttackFishBone() {
	if (shotTimer % 8 == 0) {
		if (shotTimer >= 16) {
			shotTimer = 0;
		}
