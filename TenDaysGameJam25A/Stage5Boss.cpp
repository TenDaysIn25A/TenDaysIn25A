#include "Stage5Boss.h"

Stage5Boss::Stage5Boss() { Initialize(); }

void Stage5Boss::Initialize() {
	speed = 10.0f;
	width = 128.0f;
	height = 480.0f;
	isAlive = true;
...
void Stage5Boss::InitializeBullets(int index, const BulletConfig& bulletConfig) {
...
void Stage5Boss::SetCamera(const Transform2D& camera) { renderer.SetCamera(camera); }

void Stage5Boss::Move() {

}

void Stage5Boss::TakeDamage(int damage) {
	if (!isAlive) {
		return;
	}
...
void Stage5Boss::Shot() {
	switch (attack) {
	case Stage5BossAttack::WALL:
		AttackWall();
		break;
	case Stage5BossAttack::MACHINGUN:
		AttackMachingun();
		break;
	case Stage5BossAttack::TUNNEL:
		AttackFishBone();
		break;
	case Stage5BossAttack::ALL_WALL:
		AttackAllWall();
		break;
	case Stage5BossAttack::FOURWALL:
		AttackFourWall();
		break;
	}
...
void Stage5Boss::CommonAttackSelect() {
	switch (attackPhase) {
	case AttackPhase::FIRST:
		attack = Stage5BossAttack::WALL;
		break;
	case AttackPhase::SECOND:
		attack = Stage5BossAttack::MACHINGUN;
		break;
	case AttackPhase::THIRD:
		attack = Stage5BossAttack::FOURWALL;
		break;
	}
}

void Stage5Boss::SpecialAttackSelect() {
	int randomAttack;
	switch (attackPhase) {
	case AttackPhase::FIRST:
		...
		if (randomAttack == 1) {
			attack = Stage5BossAttack::ALL_WALL;
		} else if (randomAttack == 2) {
			attack = Stage5BossAttack::TUNNEL;
		} else {
		}
		break;
	case AttackPhase::SECOND:
...
void Stage5Boss::AttackFourWall() {
	randomPositionY;
...
void Stage5Boss::AttackMachingun() {
	if (shotTimer >= 40) {
		shotTimer = 0;
...
void Stage5Boss::AttackFishBone() {
	if (shotTimer % 10 == 0) {
		if (shotTimer >= 20) {
			shotTimer = 0;
		}
